...
| `hardware/easyeda/`, `board/` | UNO Q shield PCB design (EasyEDA Pro; latest project `board/Ichiping uno q.eprj2`). Designed by an AI assistant as an experiment: the schematics are not very readable, but the circuits are correct and the boards work. |
| `hardware/3d_parts/` | STL files for the 3D-printed model house |
| `hardware/bom.*`, `docs/pins.xlsx` | shield BOM and the original pin sheet referenced by the shield specification |
| `pc/captures/full_32_eval_v1/` | small original-dataset fixture used by `test_packaged_model.py` |

The FRDM-MCXN947 firmware, its documents and datasets live in the original project: https://github.com/airpocket-soundman/IchiPing.
