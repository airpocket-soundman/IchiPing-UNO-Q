"""v6-v11 + 強化 augmentation + per-frame 正規化 (noise_diff_norm) 全自動パイプライン。

意図:
  - 学習データに v11 (高ノイズ環境採取) を加えて noise robust 化
  - per-frame zero-mean unit-variance 正規化で「形状のみ学習」させて推論時の
    SPK 音量自由度を確保 (vol up で SNR 稼げる運用に対応)
  - 強化 aug は v678910_strongaug と同じ

データ: v6-v11 6 セッション、cross-baseline 6 種。
firmware 側にも ichp_features_normalize_frame を必ず入れた build とペアで使う。
"""
...
RUN_OUT = Path("runs/neutron_v6_11_strongaug_norm_XL")
...
print("=" * 60)
print("[DONE] 強化 aug + 正規化学習 + WSL Neutron 変換完了")
print(f"       deployable: {RUN_OUT}/deploy4d/pinto_neutron_sdk26_03.tflite")
print("       firmware 側 ichp_features_normalize_frame() を有効にした build とペアで焼くこと")
print("=" * 60)
