"""v6+v7+v8+v9+v10 全自動パイプライン: 学習 → ONNX → PINTO INT8 → WSL SDK_26_03 Neutron。
...
採取済前提: pc/captures/full_32_train_v{6,7,8,9,10} (各 1600 frame)。

最終ステップで WSL SDK_26_03 経由の Neutron 変換 (7/7 = 100% NPU、firmware 互換)
を出して model_data.h 候補を生成するが flash は行わない。
"""
...
RUN_OUT = Path("runs/neutron_v678910_XL")
...
if (RUN_OUT / "best.pt").exists():
    ...
    print("=" * 60); print("[1] TRAINING v678910"); print("=" * 60)
...
# 一時 Python スクリプトを WSL から見える場所に書いて -c 経由の quote 衝突を回避
