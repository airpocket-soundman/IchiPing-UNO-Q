"""v6-v10 + 強化 augmentation 全自動パイプライン。

v678910 通常版 (test 100% / MCU 75%) の汎化不足を補うため強化 aug で再学習:
  - TimeShift ±20ms, LevelJitter ±4dB
  - NoiseOverlay SNR 0-35dB p=0.9 (信号と等強度の雑音まで経験)
  - GaussianHiss SNR 10-40dB p=0.5 (合成雑音追加)
  - FreqMask max 80 bin × 3 個 p=0.85
  - SpectralJitter σ=0.6dB p=0.9

データは v6-v10 5 セッション (= v678910 と同じ)、cross-baseline 5 種。
"""
...
RUN_OUT = Path("runs/neutron_v678910_strongaug_XL")
...
if (RUN_OUT / "best.pt").exists():
    ...
    print("=" * 60); print("[1] TRAINING v678910 STRONG AUG"); print("=" * 60)
...
# ---- 4. WSL SDK_26_03 Neutron converter (一時 .py 経由で shell quote 衝突回避) ----
...
print("=" * 60)
print("[DONE] 強化 aug 学習 + WSL Neutron 変換完了")
