"""v21-v22 (アクリル天井 era) + 強化 augmentation + per-frame 正規化 全自動パイプライン。

天井を 3D プリンタ製からアクリル樹脂プレートに変更したため、v6-v12 (旧 era) を
含めず v21+v22 のみで学習する。recipe は v6_12 strongaug norm を踏襲。

データ: v21-v22 2 セッション (3200 frames)、cross-baseline 2 種、strong aug、noise_diff_norm。
...
RUN_OUT = Path("runs/neutron_v21_22_strongaug_norm_XL")
...
if (RUN_OUT / "best.pt").exists():
    ...
    print("=" * 60); print("[1] TRAINING v21-22 STRONG AUG + NORM (acrylic ceiling)"); print("=" * 60)
...
print("=" * 60)
print("[DONE] v21-22 (acrylic) 強化 aug + 正規化学習 + WSL Neutron 変換完了")
