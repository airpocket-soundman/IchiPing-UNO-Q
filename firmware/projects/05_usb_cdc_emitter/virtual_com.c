...
/* IchiPing-side handshake flag: set when host opens the COM port (DTR=1),
 * cleared when host closes it. Polled by main.c to know when the bulk-IN
 * pipe is ready for ICHP frame writes. */
volatile uint8_t g_UsbCdcVcomReady = 0U;
...
/* Data buffer for receiving. (s_currSendBuf / s_sendSize removed by IchiPing
 * port: the example's echo loop has been replaced by main.c's frame writer.) */
    ...
    defined(FSL_FEATURE_USB_KHCI_USB_RAM) && (FSL_FEATURE_USB_KHCI_USB_RAM > 0U)
        ...
        case kUSB_DeviceCdcEventSetControlLineState:
        {
            ...
            if (acmInfo->dteStatus & USB_DEVICE_CDC_CONTROL_SIG_BITMAP_DTE_PRESENCE)
            {
                ...
                /* IchiPing: host opened the COM port → bulk-IN pipe is ready. */
                g_UsbCdcVcomReady = 1U;
            }
            /* Com port of terminal tool running on PC is closed now */
            else
            {
                acmInfo->uartState &= (uint16_t)~USB_DEVICE_CDC_UART_STATE_RX_CARRIER;
                /* IchiPing: host closed the COM port → stop sending. */
                g_UsbCdcVcomReady = 0U;
            }
...
/* CDC_VCOM_BMEnterCritical / CDC_VCOM_BMExitCritical removed by IchiPing
 * port — the only caller was the deleted APPTask echo helper. If you re-enable
 * an echo path, restore them along with the relevant DisableGlobalIRQ wrap. */
 ...
 * @return None.
 */
/* IchiPing: renamed from static APPInit, exposed so main.c can call it. */
...
/* IchiPing: APPTask (the SDK example's echo loop) and the example's main()
 * have been removed. The IchiPing main.c in this project drives its own send
 * loop, pushing ICHP frames to USB_CDC_VCOM_BULK_IN_ENDPOINT, and calls
 * USB_DeviceTask() (defined below) to keep the USB stack serviced. */

void USB_DeviceTask(void *handle_unused)
{
    (void)handle_unused;
#if USB_DEVICE_CONFIG_USE_TASK
