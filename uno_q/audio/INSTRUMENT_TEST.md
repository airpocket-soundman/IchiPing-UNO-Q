...
一意の録音ファイルを作る。当時の500ms再起動予約下でも実測クロックは約1.02秒続いた。
再生＋録音でも基準PCM一致。詳細と計測器警告は上記監査記録を参照。

2026-09-11追記: GPIO28のSD_MODE制御下で、生成サイン波がきれいに聞こえ、
発音が0.5秒で停止することをユーザーが確認した。通常試験は強制再起動を行わず、
終了・シグナル・エラー時に再生/録音プロセス停止、route off/verify、SD Low、LED消灯を行う。
生成音の上限0.5秒は維持する。過去の再起動付き測定値は履歴としてのみ扱う。
...
- ドライバー更新・置換、ファーム更新、プラグイン導入は今回行っていない。
  ...
  armed.jsonの未トリガー状態を確認してから、別途UNO Qの上限0.5秒試験を開始する。
...
長い受動記録と、UNO Qの0.5秒生成音上限は別。記録開始時刻だけで同期済みと扱わず、
...
このモードはUSB識別照合・PCM検証後、再生ルートのみを開き、録音ルートは開かない。
通常試験では直接リセットを予約しない。EXIT・シグナル・エラーを含む終了処理で
再生/録音プロセス停止、route off/verify、SD Low、LED消灯を行う。
生成音は0.5秒を上限とするが、ゼロPCM自体を物理ミュートとは扱わない。
